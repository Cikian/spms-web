    ...
    <el-table-column min-width="150px" align="center" prop="demandStatus" label="状态" class-name="table-statue-column">
      <template #default="scope">
        <el-select
            ...
            :disabled="scope.row.demandStatus===-2 || scope.row.demandStatus===-3 || scope.row.demandStatus === -9"
        >
          <el-option
...
import {
  addComment,
  deleteDemandById,
  getAllDemandByCreateId,
  getAllDemandByHeadId,
  getAllDemandByProId,
...
const getDemandsList = (proId) => {
  getAllDemandByCreateId(proId).then((res) => {
    if (res.data.code === 2001) {
