<template>
  ...
  >
    ...
    <template #tooltip>
      ...
      <div>存档测试计划前，进度必须达到已完成，并且测试报告审核通过</div>
    </template>
  </a-float-button>
</template>
...
import {
  addTestCase,
  addTestPlan,
  deleteTestCaseById, deleteTestReportById, finishTestPlan,
  queryTestCaseById,
    ...
    deleteTestCaseById(row.testCaseId)
        .then(res => {
          if (res.data.code === 200) {
            ...
            getTestCaseData()
            getTestPlanDetailById(echoTestPlan.value.testPlanId)
          } else {
            ElNotification({
              title: '提示',
              message: res.data.message,
...
const archiveTestPlan = () => {
  ElMessageBox.confirm(
      ...
      .then(() => {
        ...
        let testPlanId = echoTestPlan.value.testPlanId
        finishTestPlan(testPlanId)
            .then(res => {
              if (res.data.code === 200) {
                ElNotification({
                  title: '成功',
